...
check(
  ...
  "RideRequestIncoming should use one canonical accept pattern for ride/delivery/shared"
);
check(
...
check(
  ...
  "constants.ts must keep the private ride route backbone complete"
);
check(
  "Private ride stage screens all route through builders",
...
check(
  "Active shared trip binds all progression actions and completion persistence",
...
check(
  ...
  storeContext.includes("const completeActiveTrip = useCallback(() => {") &&
    storeContext.includes("jobType: \"ride\"") &&
    storeContext.includes("category: \"ride\""),
  "Private completion must persist status + trip record + ride revenue event"
);
check(
...
check(
  "Completion screen routes shared trips to shared detail history",
  tripCompletion.includes("/driver/history/shared/${completedTripId}"),
  "Shared completion CTA should land on shared details route"
);
...
check(
  ...
  "RideHistory entries should use route builders, not fragile string literals"
);
check(
...
// Role/shared eligibility gating
check(
