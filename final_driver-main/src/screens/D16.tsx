...
function FleetVehicleCard({ icon: Icon, title, subtitle, tag, status, onClick }: { icon: React.ElementType; title: string; subtitle: string; tag?: string; status: string; onClick?: () => void }) {
  const statusTone =
    status === "Active"
  ...
  return (
    <button onClick={onClick} className="w-full rounded-2xl border border-slate-100 bg-white px-3 py-3 shadow-sm active:scale-[0.97] transition-transform flex items-center justify-between">
        ...
        <main className="flex-1 px-4 pb-4 space-y-4 overflow-y-auto scrollbar-hide">
          ...
          <section className="rounded-2xl bg-[#0b1e3a] text-white p-4 space-y-2">
            <div className="flex items-center space-x-2">
              ...
              <div className="flex flex-col">
                ...
                <p className="text-xs font-semibold">GreenFleet Logistics · Kampala</p>
              </div>
            </div>
          ...
          <section className="space-y-2 pt-1 pb-4">
            ...
            <button
              ...
              className="w-full rounded-full py-2.5 text-sm font-semibold shadow-sm bg-white text-red-600 border border-red-200 flex items-center justify-center"
            >
