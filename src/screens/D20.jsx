import React, { useState } from "react";
import {
  Bell,
  BookOpen,
  LineChart,
  TrendingUp,
  Target,
  Clock,
  Home,
  Briefcase,
  Wallet,
  Settings,
} from "lucide-react";
import { useNavigate } from "react-router-dom";

// EVzone Driver App – D20 Preferences – Driver Info Tutorial (Boost Your Earnings) (v1)
// Earnings-focused tutorial entry: surge, peak hours, goals, cancellations.
// 375x812 phone frame, swipe scrolling in <main>, scrollbar hidden.

function BottomNavItem({ icon: Icon, label, active }) {
  return (
    <button
      className={`flex flex-col items-center justify-center flex-1 py-2 text-xs font-medium transition-colors ${
        active ? "text-[#03cd8c]" : "text-slate-500 hover:text-slate-700"
      }`}
    >
      <Icon className="h-5 w-5 mb-0.5" />
      <span>{label}</span>
    </button>
  );
}

function TipCard({ icon: Icon, title, text, badge }) {
  return (
    <div className="rounded-2xl border border-slate-100 bg-white px-3 py-3 shadow-sm flex items-start space-x-2">
      <div className="mt-0.5 flex h-8 w-8 items-center justify-center rounded-full bg-slate-50">
        <Icon className="h-4 w-4 text-[#03cd8c]" />
      </div>
      <div className="flex-1 flex flex-col items-start space-y-1">
        <div className="flex items-center space-x-2">
          <span className="text-xs font-semibold text-slate-900">{title}</span>
          {badge && (
            <span className="inline-flex items-center rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-medium text-emerald-700">
              {badge}
            </span>
          )}
        </div>
        <span className="text-[11px] text-slate-600">{text}</span>
      </div>
    </div>
  );
}

export default function DriverInfoTutorialEarningsScreen() {
  const [nav] = useState("settings");
  const navigate = useNavigate();

  return (
    <div className="min-h-screen flex justify-center bg-[#0f172a] py-4">
      {/* Local style: hide scrollbars but keep swipe scrolling */}
      <style>{`
        .scrollbar-hide::-webkit-scrollbar { width: 0; height: 0; }
        .scrollbar-hide { -ms-overflow-style: none; scrollbar-width: none; }
      `}</style>

      <div className="w-[375px] h-[812px] bg-white rounded-[32px] shadow-2xl overflow-hidden flex flex-col">
        {/* Header */}
        <header className="flex items-center justify-between px-4 pt-4 pb-2">
          <div className="flex items-center space-x-2">
            <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#e6fff7]">
              <BookOpen className="h-4 w-4 text-[#03cd8c]" />
            </div>
            <div className="flex flex-col items-start">
              <span className="text-[11px] uppercase tracking-[0.18em] text-slate-400">
                Preferences · Training
              </span>
              <h1 className="text-base font-semibold text-slate-900">
                Boost your earnings
              </h1>
            </div>
          </div>
          <button className="relative inline-flex h-9 w-9 items-center justify-center rounded-full bg-slate-100 text-slate-700">
            <Bell className="h-4 w-4" />
            <span className="absolute -top-0.5 -right-0.5 inline-flex h-3 w-3 items-center justify-center rounded-full bg-[#f77f00]" />
          </button>
        </header>

        {/* Content */}
        <main className="flex-1 px-4 pb-4 space-y-4 overflow-y-auto scrollbar-hide">
          {/* Hero card */}
          <section className="rounded-2xl bg-[#0b1e3a] text-white p-4 space-y-3">
            <div className="flex items-center space-x-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#03cd8c] text-slate-900">
                <LineChart className="h-5 w-5" />
              </div>
              <div className="flex flex-col">
                <span className="text-[10px] tracking-[0.18em] uppercase text-[#a5f3fc]">
                  Earnings tutorial
                </span>
                <p className="text-sm font-semibold">
                  Learn how to earn smarter with EVzone.
                </p>
              </div>
            </div>
            <p className="text-[11px] text-slate-100 leading-snug">
              This tutorial shows how peak hours, surge pricing, trip choices
              and cancellations affect your earnings – and how to set realistic
              weekly goals as an EV driver.
            </p>
            <div className="flex items-center space-x-2 text-[10px] text-emerald-300">
              <Clock className="h-3.5 w-3.5" />
              <span>Estimated time: 6–8 minutes</span>
            </div>
          </section>

          {/* Tips & mini lessons */}
          <section className="space-y-2">
            <h2 className="text-sm font-semibold text-slate-900 mb-1">
              Key lessons in this tutorial
            </h2>
            <TipCard
              icon={TrendingUp}
              title="Use peak hours wisely"
              text="See when demand is highest in your city and how to position yourself without chasing every surge."
              badge="Peak hours"
            />
            <TipCard
              icon={Target}
              title="Set and track weekly goals"
              text="Learn how to set realistic income targets and track them from the Earnings & Goals screen."
              badge="Goals"
            />
            <TipCard
              icon={LineChart}
              title="Manage cancellations"
              text="Understand how frequent cancellations and no-shows can reduce your incentives over time."
              badge="Cancellations"
            />
          </section>

          {/* CTA buttons */}
          <section className="pt-1 pb-4 flex flex-col space-y-2">
            <button
              type="button"
              onClick={() => navigate("/driver/training/quiz")}
              className="w-full rounded-full py-2.5 text-sm font-semibold shadow-sm bg-[#03cd8c] text-slate-900 hover:bg-[#02b77c] flex items-center justify-center"
            >
              <LineChart className="h-4 w-4 mr-2" />
              Start earnings tutorial
            </button>
            <button
              type="button"
              onClick={() => navigate("/driver/preferences")}
              className="w-full rounded-full py-2.5 text-sm font-semibold border border-slate-200 text-slate-800 bg-white"
            >
              I’ll do this later
            </button>
            <p className="text-[10px] text-slate-500 text-center">
              You can revisit this tutorial any time from Preferences → Training &
              learning.
            </p>
          </section>
        </main>

        {/* Bottom navigation – Settings active (Preferences context) */}
        <nav className="border-t border-slate-100 bg-white/95 backdrop-blur flex">
          <BottomNavItem icon={Home} label="Home" active={nav === "home"} />
          <BottomNavItem icon={Briefcase} label="Manager" active={nav === "manager"} />
          <BottomNavItem icon={Wallet} label="Wallet" active={nav === "wallet"} />
          <BottomNavItem icon={Settings} label="Settings" active={nav === "settings"} />
        </nav>
      </div>
    </div>
  );
}
