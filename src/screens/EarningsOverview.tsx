...
import { getDriverWalletSummary } from "../services/api/driverApi";
...
export default function EarningsOverview() {
  ...
  useEffect(() => {
    ...
    void getDriverWalletSummary()
      ...
      .catch((error) => {
        console.warn("Failed to load driver wallet summary.", error);
      });
    return () => {
      cancelled = true;
    };
  ...
  const sessionTotal = recentEarnings.reduce((sum, entry) => sum + entry.amount, 0);
  ...
  const lifetimeDistanceKm = useMemo(
    () =>
      trips.reduce((sum, trip) => {
        const match = String(trip.distance || "").match(/(\d+(?:\.\d+)?)/);
  ...
  const trackedHours = useMemo(
    () =>
      trips.reduce((sum, trip) => {
        ...
        return sum + (trip.completedAt - trip.startedAt) / 3600000;
      }, 0),
    [trips],
  );
      ...
      <main className="flex-1 px-6 pt-6 pb-24 space-y-8 overflow-y-auto scrollbar-hide">
        ...
        <section className="rounded-[2.5rem] border-2 border-emerald-500/10 bg-white p-7 space-y-8 shadow-sm hover:border-emerald-500/30 transition-all">
          <div className="flex items-center justify-between">
            <div className="flex flex-col">
               ...
               <span className="text-[11px] font-black uppercase tracking-widest text-slate-900">Live Trend</span>
            </div>
          ...
          <div className="flex flex-col items-center space-y-2">
            ...
            <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest opacity-60">Session Total: {formatUGX(sessionTotal)}</div>
          </div>
        ...
        */}
        ...
        <section className="space-y-4">
           ...
           <div className="grid grid-cols-2 gap-3">
             <MetricCard label="Total Trips" value={String(trips.length)} icon={History} onClick={() => navigate("/driver/history/rides")} />
             <MetricCard label="Dist. Covered" value={`${lifetimeDistanceKm.toFixed(1)} km`} icon={History} />
             <MetricCard label="Hours Online" value={`${trackedHours.toFixed(1)}h`} />
             <MetricCard label="Completed" value={String(completedTrips)} />
           </div>
        </section>
        ...
        <section className="space-y-6 pt-4 pb-12">
          ...
          <div className="rounded-[2rem] border border-slate-100 bg-white p-5 text-center">
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">
              Live station demand analytics are not yet exposed by the backend.
            </p>
          </div>
        </section>
