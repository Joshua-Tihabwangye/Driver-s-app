...
function JobMixPill({ icon: Icon, label, value, colorClass }) {
  return (
    <div
      className={`flex flex-col items-start rounded-2xl border px-3 py-2 text-[11px] ${colorClass}`}
    >
    ...
    <div className="min-h-screen flex justify-center bg-[#0f172a] py-4">
      ...
      <div className="w-[375px] h-[812px] bg-white rounded-[32px] shadow-2xl overflow-hidden flex flex-col">
        ...
        <nav className="border-t border-slate-100 bg-white/95 backdrop-blur flex">
          ...
          <BottomNavItem icon={Car} label="Jobs" active={nav === "manager"} />
          <BottomNavItem icon={DollarSign} label="Wallet" active={nav === "wallet"} />
          <BottomNavItem icon={Map} label="Map" active={nav === "settings"} />
        </nav>
      </div>
    </div>
  );
