...
function positionClassToLatLng(
	...
	if (
		positionClass.includes("left-1/2") ||
		positionClass.includes("-translate-x-1/2")
	)
		x = 0.5;
	if (
		positionClass.includes("top-1/2") ||
		positionClass.includes("-translate-y-1/2")
	)
		y = 0.5;
...
function toMapMarker(marker: DriverMapMarker, center: LatLng) {
	...
	return {
		...
		position:
			marker.position ||
			positionClassToLatLng(marker.positionClass, center),
	};
}
		...
		>
				...
				{canRenderGoogleMap ? (
					...
					>
						...
						{visibleMarkers.map((marker) => {
							const palette =
								MARKER_STYLES[marker.tone || "neutral"];
						...
						{selfPosition && (
							...
							>
								<div className="relative">
									...
									/>
									{/* Main pin */}
									<div className="relative z-10 flex h-8 w-8 items-center justify-center rounded-full bg-blue-500 border-2 border-white shadow-lg">
			...
			{routePath ? (
				<div
					...
					style={{
						transform: `rotate(${mapRef ? mapRef.getHeading() || 0 : 0}deg)`,
			...
			mapLoadFailure === "offline" ? (
				<div className="absolute left-1/2 top-[74px] z-30 -translate-x-1/2 rounded-full border border-amber-300/70 bg-white/96 px-4 py-2 text-[10px] font-black uppercase tracking-[0.12em] text-amber-700 shadow-lg backdrop-blur-sm">
					Offline. Live map updates will resume when the connection
					returns.
				</div>
			) : null}
			...
			{canRenderGoogleMap && !hasGeolocation ? (
				<div className="absolute left-1/2 top-[74px] z-30 -translate-x-1/2 rounded-full border border-amber-300/70 bg-white/96 px-4 py-2 text-[10px] font-black uppercase tracking-[0.12em] text-amber-700 shadow-lg backdrop-blur-sm">
					Location services unavailable. Recenter is disabled until
					GPS is available.
				</div>
			) : null}
				...
				{bottomRightSlot || (
					<div className="shrink-0 self-start rounded-xl border border-slate-200 bg-white/95 px-3 py-2.5 text-right shadow-lg backdrop-blur-sm sm:self-auto">
						<p className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-600">
							Z{zoom} • {layerLabel.toUpperCase()} • T:
							{trafficOn ? "ON" : "OFF"} • A:
							{alertsOn ? "ON" : "OFF"}
						</p>
					</div>
